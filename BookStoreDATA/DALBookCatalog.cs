    ...
    public class DALBookCatalog
    {
        ...
        public string GetBookTitle(string isbn)
        {
            try
            {
                ...
                cmd.Connection = conn;
                cmd.CommandText = "Select Title from BookData WHERE ISBN = @ISBN";
                cmd.Parameters.AddWithValue("@ISBN", isbn);
                conn.Open();
                string title;
        ...
        public string GetBookCategoryName(int id)
        {
            try
            {
                SqlCommand cmd = conn.CreateCommand();
                cmd.Connection = conn;
                cmd.CommandText = "Select Name from Category WHERE CategoryID = @ID";
                cmd.Parameters.AddWithValue("@ID", id);
                conn.Open();
                string name;
                if (cmd.ExecuteScalar() == null)
